...
- **Async tests are refused, not supported.** `test()` fails a thenable return or an
  `AsyncFunction`, attaching `value.catch(() => {})` **before** recording the failure.
...
- **#22 owns the sanitisation-layer tests**; this issue owns the enforcement mechanism and
  the proof that it catches its own fixtures.
...
1. **`tests/harness.js`, `tests/run.js`, `tests/selftest.js`, `flake.nix`: a failing test
   fails the run.** `harness.js:25-32`: fail when `fn.constructor.name === "AsyncFunction"`
   before calling it; otherwise capture `const value = fn()` and, if it is thenable, call
   `value.catch(() => {})` and *then* `failures.push({ name, error: new Error("async test
   function; Model.js is synchronous") })`. Add `failures: failures` to the exports
   ...
   (one throwing synchronously, one `async` that rejects, one `async` that resolves), then
   sets `process.exitCode = 0` only if `harness.failures.length === 3` and
   `harness.report() === 1`. `flake.nix:93` gains `node tests/selftest.js` above
   `node tests/run.js`.
   → verify by `node tests/selftest.js; echo $?` printing `0` with no
   `UnhandledPromiseRejection` on stderr, and `node tests/run.js` still `70 passed, 0
   failed`. Runner-safe: two `node` processes over the sources the check already copies.
...
3. **`flake.nix`: widen the colour check.** Replace `flake.nix:143-145` with four greps
   ...
   → verify by the four fixtures under Tests, and by the untouched tree passing.
...
4. **`flake.nix`, `ci.yml`: a check asserts its own subject exists.** `flake.nix:149-152`
   ...
   → verify by renaming `docs/img` away and seeing both fail. Runner-safe: `test`, `du`.
...
6. **`flake.nix`, `AGENTS.md`, `README.md`, `ci.yml`: one source of truth, checked.**
   Every check block gets a `# check: <name>` first line — `tests`, `manifest`,
...
commit. Nothing below is committed: the proof is a transcript in the PR body, never a
violation left in the repository.
...
the previous behaviour returns exactly. The only runtime change anywhere is
...
failures and a `run.js` that calls `process.exit()`, and `tests/selftest.js` then fails —
so revert step 1 and the `flake.nix:93` self-test line together, or not at all.

A PR green under the new checks stays green after a revert: every check is strictly
stricter than the one it replaces. The reverse does not hold — a branch cut before this
lands may fail the files-list, colour or pacman check on merge. That is the mechanism
working, and the fix is one line in `files`, one `Color.*` token, or one line in
`tests/pacman-allowed.txt`.
